...
from zerok.utils.curve import Scalar, G1Point, ec_lincomb
from zerok.utils.poly import Polynomial, Basis
from zerok.lookup.plookup.program import Params, aggregate, aggregate_comm
from zerok.lookup.plookup.prover import Proof
from zerok.lookup.plookup.setup import Setup
from zerok.lookup.plookup.transcript import Transcript
...
class Verifier:
    ...
    def verify(self, pf: Proof) -> bool:
        ...
        def get_b():
            ...
            lhs = (
                ...
                * (Scalar(1) + beta)
                * (gamma + f_eval)
                * (gamma * (Scalar(1) + beta) + t_eval + beta * t_g_eval)
            )
            rhs = (
                (zeta - self.H[self.n])
                * z_g_eval
                * (gamma * (1 + beta) + h1_eval + beta * h1_g_eval)
        ...
        agg_comm = aggregate_comm(
            eps, [f_comm, h1_comm, h2_comm, z_comm, q_comm]
        )  # agg(tau)
        agg_eval = aggregate(
            eps, [f_eval, h1_eval, h2_eval, z_eval, q_eval]
        )  # agg(zeta)
    ...
    def check_pairing(
        self, comm: G1Point, eval: Scalar, witness_comm: G1Point, point: Scalar
    ):
        # comm: P(tau)
        # eval: P(point)
