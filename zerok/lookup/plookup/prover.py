...
from zerok.utils.curve import Scalar
from zerok.lookup.plookup.program import Params, aggregate, aggregate_poly
from zerok.lookup.plookup.setup import Setup
from zerok.lookup.plookup.transcript import (
    Transcript,
    Message1,
    Message2,
    Message3,
...
class Prover:
    ...
    def __init__(self, setup: Setup, params: Params):
        ...
        self.H = params.roots
        # vanish = x^(n+1) - 1
        self.vanish = Polynomial(
            [Scalar(-1)] + [Scalar(0)] * self.n + [Scalar(1)], Basis.MONOMIAL
        )
    ...
    def round_5(self):
        agg_poly = aggregate_poly(
            self.eps, [self.f_poly, self.h1, self.h2, self.z, self.q]
        ).to_mononial()
        agg_eval = aggregate(
            self.eps,
            [self.f_eval, self.h1_eval, self.h2_eval, self.z_eval, self.q_eval],
        )
        self.agg_witness = (agg_poly - agg_eval) / Polynomial(
            [-self.zeta, Scalar(1)], Basis.MONOMIAL
        )
        ...
        agg_g_eval = aggregate(
            self.eps, [self.h1_g_eval, self.h2_g_eval, self.z_g_eval]
        )
        self.agg_g_witness = (agg_g_poly - agg_g_eval) / Polynomial(
            [-self.g_zeta, Scalar(1)], Basis.MONOMIAL
        )
    ...
    def compute_polynomial(self, beta: Scalar, gamma: Scalar):
        ...
        for i in range(1, self.n):
            ...
            denom = (
                denom
                * (gamma * (Scalar(1) + beta) + self.s[i - 1] + beta * self.s[i])
                * (
                    gamma * (Scalar(1) + beta)
                    + self.s[self.n + i - 1]
                    + beta * self.s[self.n + i]
                )
            )
    ...
    def quotient_poly(self) -> Polynomial:
        L0 = Polynomial(
            [Scalar(1) if i == 0 else Scalar(0) for i in range(self.n + 1)],
            Basis.LAGRANGE,
        ).ifft()  # same as L_1(x) in paper
        Ln = Polynomial(
            [Scalar(1) if i == self.n else Scalar(0) for i in range(self.n + 1)],
            Basis.LAGRANGE,
        ).ifft()  # same as L_{n+1}(x) in paper
        Z = self.z.ifft()
        # L0(x)(Z(x)-1)
    ...
    def get_poly_b(self) -> Polynomial:
        ...
        lhs = (
            ...
            * self.z.ifft()
            * (Scalar(1) + self.beta)
            * (self.f_poly.ifft() + self.gamma)
            * (
                self.t_poly
                + self.t_poly.shift(1) * self.beta
                + self.gamma * (Scalar(1) + self.beta)
            ).ifft()
        )
        rhs = (
            front
            ...
            * (
                self.h1
                + self.h1.shift(1) * self.beta
                + self.gamma * (Scalar(1) + self.beta)
            ).ifft()
            * (
                self.h2
                + self.h2.shift(1) * self.beta
                + self.gamma * (Scalar(1) + self.beta)
            ).ifft()
        )
        return lhs - rhs
