...
from zerok.utils.curve_optimized import (
    ec_lincomb,
    G1Point,
    G2Point,
    Scalar,
...
from zerok.utils.poly_optimized import Polynomial, Basis
...
class KZGSetup(object):
    ...
    def open(self, poly: Polynomial, x: Scalar) -> Tuple[Scalar, G1Point]:
        v = poly.eval(x)
        if poly.basis == Basis.LAGRANGE:
            poly = poly.ifft()
