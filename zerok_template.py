...
def create_zerok_compiler_file_content(
    dag: str, num_variables: int, output: int, witness: list[int]
) -> str:
    witness = parse_witness_list(witness)
    output = f'BN254::from(bigint_to_fr("{output}"))'
    return rust_zerok_compiler.format(
        dag=dag, num_variables=num_variables, output=output, witness=witness
    )


def parse_witness_list(witness_list: list[int]) -> str:
    return ",\n            ".join(
        [f'BN254::from(bigint_to_fr("{witness}"))' for witness in witness_list]
    )


def execute_zerok_compiler():
    subprocess.call("cargo run --bin zerok", shell=True)
